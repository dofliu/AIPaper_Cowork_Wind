...
**G3：`CONTRACT_RATIFIED`**（2026-08-21）——
`NOT_COMPARABLE` 的風險由 G1 轉移到這裡（3.4a 末），三項狀態契約已寫成
[`docs/method/POGO_G3_STATE_CONTRACT.md`](POGO_G3_STATE_CONTRACT.md)，
唯一的取捨經 **R28 裁決（A 案：4 組宣告設定）**。**現在可以建環境了**
...
（草案 v1.0，狀態 `RATIFICATION_PENDING`）。三項全部以文字寫死：
   ...
   ~~**唯一需要裁決的是宣告設定要 2 組還是 4 組**~~
   **✅ 劉老師 2026-08-21 已裁決（R28：A 案）——`k ∈ {4,5}` × `carry ∈
   {none, within_farm}` 共 4 組全部跑，POGO 的頭號數字取較好者。**
   **執行矩陣已定死，可以建環境了。** 呈報時**必須寫明頭號數字是 4 組的最大值
   並附上全部 4 組**（見契約第 5 節；不說 N 的 max over N 就是自製的
...
~~**執行 owner：TBD**（R26 第 8 節；截至 **2026-08-21** 仍未指派）。~~
**✅ 執行 owner：automation-4**（劉老師 **2026-08-22 00:42** 指派，
留痕在 Drive《開發日誌 v5.3》文末的裁決補記段）。
R26 狀態 → **`OWNER_ASSIGNED / EXECUTION_NOT_STARTED`**。

G0／G1／G3 都已完成，而 R28 裁決之後**不再有任何裁決擋在前面**——
但那三關**都是閱讀、核對與規格工作**，從第 1 項起需要的是**建環境與實作**。
**指派解除的是阻塞，不是工作：G5–G8 仍全部 `NOT RUN`。**

**執行順序、每一關的驗收條件與跑完要交什麼，見
[`POGO_R26_EXECUTION_RUNBOOK.md`](POGO_R26_EXECUTION_RUNBOOK.md)**
（該手冊不重述規格，只寫順序與權威在哪一份）。
每次執行的 receipt 由 `scripts/check_pogo_receipt.py` 機器檢查——
契約第 6 節要求留 receipt，而**一份沒有人檢查的 receipt 只是文書工作**。
